...
TEST(EnumLibTest, EnumerateUserRights_Administrator) {
    ...
    LSA_HANDLE PolicyHandle = NULL;
    GetLSAPolicyHandleAllAccess(L".", &PolicyHandle);
    std::wstring userName = L"Administrator"; // Replace with a valid user name for testing
    ...
    EXPECT_FALSE(EnumerateUserRights(PolicyHandle, L"Guest").empty());
    EXPECT_FALSE(EnumerateUserRights(PolicyHandle, L"Administrator").empty());
